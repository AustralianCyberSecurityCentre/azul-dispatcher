...
func TestCacheExtended(t *testing.T) {
	...
	store, err = NewDataCache(1, 300, 256, store)
...
func TestCache(t *testing.T) {
	...
	store, err = NewDataCache(10, 300, 256, store)
	if err != nil {
	...
	for _, table := range tables {
		...
		_, err = f.Write(table.input)
		if err != nil {
			t.Fatalf("Failed to write input data to temp file %s", err.Error())
		}

		err = store.Put("source", "label", table.sha256, f.Name(), int64(len(table.input)))
		if err != nil {
			t.Error("Failed to put data", err)
		}

		// now fetch the object back
		ds, err := store.Fetch("source", "label", table.sha256, 0, -1)
		if err != nil {
		...
		ds, err = store.Fetch("source", "label", table.sha256, 1, -1)
		if err != nil {
		...
		ds, err = store.Fetch("source", "label", table.sha256, int64(size)+1, 1)
		if err == nil {
		...
		ds, err = store.Fetch("source", "label", table.sha256, -int64(size*2), -1)
		if err != nil {
